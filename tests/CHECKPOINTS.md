...
| `cp-water-column`     | dev   | <1 s     | Column UI math: slice-at-depth geometry (shallow shelf grows no murk; deep bottoms clip + flag), two-number spot formatting, crossing callout, diurnal-strip normalization | Rendered pixels (web-smoke boots the real widget)            |
