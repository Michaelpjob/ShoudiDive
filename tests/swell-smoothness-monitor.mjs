...
// decay, a 12 ft groundswell drops by ~0.6 ft per cell (5%/cell). Allow
// 2.5 ft as a soft cap — well below the ~10 ft jump the broken algorithm
// produces, but tolerant of legitimate physical gradients (e.g. near a
// breakwater or in a narrow channel).
const MAX_DELTA_FT = 2.5;
...
// Each transect: {name, lat0, lng0, lat1, lng1}.
const TRANSECTS = [
  // Pacific Baja northern seam — Cedros + Isla San Benito offshore
  // edge. Crosses the WW3 mask boundary west→east at the latitude
  // where the user reported a visible hard line in the screenshot.
  { name: "Cedros offshore east-west",      lat0: 28.5, lng0: -118.0, lat1: 28.5, lng1: -114.5 },
  // Vizcaíno peninsula coastal seam — Bahía Tortugas.
  { name: "Tortugas offshore east-west",    lat0: 27.7, lng0: -117.5, lat1: 27.7, lng1: -114.5 },
  // Northern Pacific Baja — near international border.
  { name: "Northern Baja east-west",        lat0: 30.5, lng0: -117.8, lat1: 30.5, lng1: -116.0 },
  // Magdalena Bay area.
  { name: "Magdalena Bay east-west",        lat0: 24.5, lng0: -114.0, lat1: 24.5, lng1: -111.0 },
  // South tip — Cabo Falso to Cabo Pulmo (crosses Pacific→Cortez via
  // the cape; expect a real physical gradient here but no single-cell
  // cliffs).
  { name: "Cabo south-rounding",            lat0: 22.8, lng0: -110.2, lat1: 23.4, lng1: -109.3 },
  // North Cortez — Bahía de Los Ángeles to Tiburón.
  { name: "Midriff islands east-west",      lat0: 28.9, lng0: -113.5, lat1: 28.9, lng1: -112.0 },
  // Central Cortez — La Paz / Espíritu Santo east-west.
  { name: "La Paz to mainland east-west",   lat0: 24.5, lng0: -110.5, lat1: 24.5, lng1: -107.5 },
];
...
async function probe(page, url) {
  return page.evaluate(async ({ url, transects, gridW, gridH, bbox, heightRange }) => {
    ...
    function walk(t) {
      ...
      return { name: t.name, samples, maxDelta, maxAt, interiorNaN,
...
async function main() {
  ...
  try {
    ...
    for (const r of out.results) {
      const flag = r.maxDelta > MAX_DELTA_FT;
      const status = flag ? "FAIL" : "ok  ";
      const at = r.maxAt
        ? ` worst ${r.maxAt.from.hsFt}→${r.maxAt.to.hsFt} ft at ${r.maxAt.to.lat},${r.maxAt.to.lng}`
        : " no valid samples";
      console.log(`  ${status} ${r.name.padEnd(36)} Δmax=${r.maxDelta.toFixed(2)} ft  interior-NaN=${r.interiorNaN}/${r.cells}${at}`);
      if (flag) failures.push(r);
    }

    const summary = { ok: failures.length === 0, url, max_delta_ft: MAX_DELTA_FT,
    ...
    if (failures.length === 0) {
      console.log(`PASS — all ${out.results.length} transects smooth`);
      exit = 0;
    } else {
      console.log(`FAIL — ${failures.length}/${out.results.length} transects have sharp seams`);
      exit = 1;
    }
  } catch (e) {
